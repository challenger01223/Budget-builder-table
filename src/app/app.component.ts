...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [ContextMenuComponent, RouterOutlet, FormsModule, CommonModule],
...
export class AppComponent implements OnInit {
  ...
  showBalance = 0;
  menuItems = [{ label: 'Apply to all', action: this.applyToAll.bind(this) }];
  ...
  onMonthChange(): void {
    ...
    let col = 0;
    const sDate = new Date(this.startMonth),
      eDate = new Date(this.endMonth);
    const year1 = sDate.getFullYear(),
      month1 = sDate.getMonth(),
      year2 = eDate.getFullYear(),
      month2 = eDate.getMonth();
    this.monthCount = 0;
    for (let year = year1; year <= year2; year++)
      for (
        let month = year == year1 ? month1 : 0;
        month <= (year == year2 ? month2 : 11);
        month++
      ) {
        this.monthCount++;
        this.months.push({
  ...
  onDataChange(): void {
    ...
    for (let i = 0; i <= this.monthCount; i++) {
      ...
      this.balances[2].values[i] =
        this.balances[1].values[i] + this.balances[0].values[i];
    }
  }
    ...
    switch (event.key) {
      case 'Enter':
        if (element.getAttribute('name') != null && element.value != '') {
          ...
          if (
            element.getAttribute('name') == 'addSubIncome' ||
            element.getAttribute('name') == 'addSubExpense'
          )
            newCategory.type = 1;
          newCategory.tvalues = new Array(this.monthCount);
          newCategory.values = new Array(this.monthCount + 1);
          for (let j = 0; j <= this.monthCount; j++) newCategory.values[j] = 0;
          if (
            element.getAttribute('name') == 'addSubExpense' ||
            element.getAttribute('name') == 'addParentExpense'
          ) {
  ...
  onArrow(event: KeyboardEvent, element: HTMLElement): void {
    ...
    switch (event.key) {
      case 'ArrowUp':
        ...
        if (Number(index[0]) > 0) {
          document
            .getElementById(Number(index[0]) - 1 + '_' + index[1])
            ?.focus();
        }
        break;
      case 'ArrowDown':
        ...
        if (
          Number(index[0]) <
          this.incomes.length + this.expenses.length + this.balances.length
        ) {
          document
            .getElementById(Number(index[0]) + 1 + '_' + index[1])
            ?.focus();
      ...
      case 'ArrowLeft':
        ...
        if (Number(index[1]) > 0) {
          document
            .getElementById(index[0] + '_' + (Number(index[1]) - 1))
            ?.focus();
        }
        break;
      case 'ArrowRight':
